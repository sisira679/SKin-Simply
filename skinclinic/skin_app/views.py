...
from .forms import DoctorForm,PharmacistForm,DeliveryForm
...
def add_doctor(request):
    if request.method == 'POST':
        ...
        if doctor_form.is_valid() and specializations_form.is_valid():
            ...
            # Redirect to the doctor profile list
            return redirect('doctorprofile_list')  # Update with your actual view name
    else:
        doctor_form = DoctorForm()
        specializations_form = SpecializationsForm()
...
def service_list(request):
    ...
    user_profile, created = UserProfile.objects.get_or_create(user=request.user)
    return render(request, 'services.html', {'services': services,'user_profile': user_profile})
...
def add_delivery(request):
    if request.method == 'POST':
        ...
        if delivery_form.is_valid():
            user = delivery_form.save(commit=False)
            password = delivery_form.cleaned_data['password']
            user.set_password(password)
            user.save()
    ...
    return render(request, 'register_delivery_boy.html', {'delivery_form': delivery_form})
...
def delivery_update_status(request, order_id):
    ...
    if request.method == 'POST':
        ...
        if delivery_status == 'Delivered':
            ...
            return redirect('availableorders')  # Redirect to the delivery boy dashboard

    return render(request, "deliverupdatestatus.html", {'order': order})
